...
const FRAME_TEMPLATE = [
  { frame_order: 1, game_type: '8ball',  is_doubles: false, label: 'Singles — 8-Ball' },
  { frame_order: 2, game_type: '9ball',  is_doubles: false, label: 'Singles — 9-Ball' },
  { frame_order: 3, game_type: '9ball',  is_doubles: true,  label: 'Doubles — 9-Ball' },
  { frame_order: 4, game_type: '10ball', is_doubles: true,  label: 'Doubles — 10-Ball' },
  { frame_order: 5, game_type: '9ball',  is_doubles: false, label: 'Singles — 9-Ball' },
  { frame_order: 6, game_type: '10ball', is_doubles: false, label: 'Singles — 10-Ball' },
]
...
export default function AddResult() {
  ...
  const handleSubmit = async (e) => {
    ...
    for (const f of frames) {
      if (!f.winning_team_id) { setError('Please select a winner for every frame.'); return }
    }
    ...
    <div className="max-w-3xl mx-auto px-4 py-8 sm:py-12">
      ...
      <div className="flex items-start justify-between mb-8 flex-wrap gap-4">
        <div>
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-white tracking-tight">Add Match Result</h1>
        </div>
        {/* Step indicator */}
      ...
      {step === 1 && (
        <form onSubmit={handleStep1} className="bg-white dark:bg-[#111] border border-gray-200 dark:border-white/8 rounded-2xl p-6 flex flex-col gap-5 shadow-sm dark:shadow-none">
          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-1.5">Match Date</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-1.5">Home Team</label>
              <CustomSelect value={homeTeamId} onChange={setHomeTeamId} options={teamOptions} placeholder="Select team" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-1.5">Away Team</label>
              <CustomSelect value={awayTeamId} onChange={setAwayTeamId} options={teamOptions} placeholder="Select team" />
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-1.5">
              Notes <span className="normal-case font-normal tracking-normal text-gray-400 dark:text-gray-600">(optional)</span>
            </label>
            <input value={notes} onChange={e => setNotes(e.target.value)} placeholder="e.g. Week 3, Home venue" className={inputCls} />
          </div>
          <button
            type="submit"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="bg-white dark:bg-[#111] border border-gray-200 dark:border-white/8 rounded-2xl px-5 py-4 flex items-center justify-between shadow-sm dark:shadow-none">
            <div className="min-w-0">
              <div className="text-[10px] text-gray-400 dark:text-gray-600 uppercase tracking-widest">Home</div>
            ...
            <div className="text-center shrink-0">
              ...
              <div className="text-[10px] text-gray-400 dark:text-gray-600 uppercase tracking-widest">{completedFrames}/6 frames</div>
            </div>
            <div className="min-w-0 text-right">
              <div className="text-[10px] text-gray-400 dark:text-gray-600 uppercase tracking-widest">Away</div>
          ...
          {frames.map((frame, i) => {
            ...
            return (
              <div key={frame.frame_order}
                className={`bg-white dark:bg-[#111] border rounded-2xl p-4 sm:p-5 transition-all shadow-sm dark:shadow-none ${done ? 'border-emerald-200 dark:border-emerald-500/25' : 'border-gray-200 dark:border-white/8'}`}
              >
                ...
                {/* Winner toggle */}
                <div className="mb-4">
                  <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2">Winner</p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: `${homeTeamName} P1`, field: 'home_player1_id', options: homePlayers },
                    { label: `${awayTeamName} P1`, field: 'away_player1_id', options: awayPlayers },
                    ...(frame.is_doubles ? [
                      { label: `${homeTeamName} P2`, field: 'home_player2_id', options: homePlayers },
                      { label: `${awayTeamName} P2`, field: 'away_player2_id', options: awayPlayers },
                    ] : []),
                    ...
                    <div key={field}>
                      ...
                      <CustomSelect value={frame[field]} onChange={v => updateFrame(i, field, v)} options={options} placeholder="— none —" />
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="flex gap-3 mt-2">
            ...
            >
              ...
              ) : completedFrames < 6 ? `Save Result (${completedFrames}/6)` : 'Save Result'}
            </button>
          </div>
        </form>
      )}
